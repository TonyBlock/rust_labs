...
fn offset_differences(offset: usize, values: Vec<i32>) -> Vec<i32> {
    let first_iter = (&values).into_iter();
    let second_iter = (&values).into_iter().cycle().skip(offset);
    first_iter.zip(second_iter).map(|(first, second)| second - first).take(values.len()).collect()
}
